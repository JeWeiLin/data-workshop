...
- 前往 BigQuery 頁面，建立 dataset。後續一張表用來存放後續的 Embedding 資料，一張為表存放資料載入。

```bash
bq mk --dataset --location=asia-east1 your-project-id:your_dataset_name
```
<br>
<br>

- 在 BigQuery 頁面中左側的 Explorer 點選 Connection ，並建立連線，Connection type 選擇 Vertex AI remote models, remote functions, BigLake and Spanner (Cloud Resource) 遠端模型，輸入連線 ID。
<br>
<br>

-  前往 IAM 開啟權限。將前面創建連線的服務帳戶 ID 加入身分與存取權管理。授予 Vertex AI User 存取權。
在 Explorer 點選 Connection，會出現服務帳戶 ID 的連線資訊。
<br>
<br>

- 在 BigQuery SQL 工作區中，貼上以下指令，將會建立一個模型物件，將遠端資源與資料集連結起來。

```SQL
CREATE OR REPLACE MODEL `your-project-id.your-dataset.your-model` 
REMOTE WITH CONNECTION `your-project-id.asia-east1.your-connection-id` 
...
- 在 BigQuery 中，執行以下向量搜尋的 SQL，會在先前建立的 Dataset 中產生 Model (your-model) 與向量表 (your-embedding-table)。
...
1. ML.GENERATE_EMBEDDING (查詢向量化)：
    ...
    - 使用與建立資料表時相同的模型 (your-model)，確保兩者在相同的語意空間中比較。

2. VECTOR_SEARCH (向量比對)：

    - 目標表：your-embedding-table。
